        ...
        if (!string.IsNullOrWhiteSpace(chatInput))
        {
            ...
            if (!trimmed.StartsWith('/'))
            {
                ...
                if (target != null)
                {
                    if (Sheets.IsInForay())
                    {
                        var worldName = Sheets.WorldSheet.TryGetRow(target.World, out var forayWorld) ? forayWorld.Name.ExtractText() : string.Empty;
                        var contentIdToUse = target.ContentId;
                        if (contentIdToUse == 0)
                        {
                            var historyInfo = Plugin.Functions.Chat.GetTellHistoryInfo(0);
                            if (historyInfo != null && string.Equals(historyInfo.Name, target.Name, StringComparison.Ordinal))
                                contentIdToUse = historyInfo.ContentId;
                        }
