...
public class InputHandler
{
    ...
    private unsafe int Callback(scoped ref ImGuiInputTextCallbackData data)
    {
        ...
        {
            var bytePos = GetBytePosFromCharPos(ChatInput, Plugin.InputPreview.SelectedCursorPos);
            data.CursorPos = bytePos;
            data.SelectionStart = data.SelectionEnd = bytePos;
            Plugin.InputPreview.SelectedCursorPos = -1;
        }

        CursorPos = GetCharPosFromBytePos(ChatInput, data.CursorPos);
        if (data.EventFlag == ImGuiInputTextFlags.CallbackCompletion)
        {
            ...
            var correctedStart = GetCharPosFromBytePos(ChatInput, white + 1);
            var correctedCursor = correctedStart + (utf8Message?.Length ?? 0);
            AutoCompleteHandler.AutoCompleteInfo = new AutoCompleteInfo(utf8Message ?? string.Empty, correctedStart, correctedCursor);
            AutoCompleteHandler.AutoCompleteOpen = true;
            AutoCompleteHandler.AutoCompleteSelection = 0;
            return 0;
        }
        ...
        {
            Activate = false;
            data.CursorPos = GetBytePosFromCharPos(ChatInput, ActivatePos > -1 ? ActivatePos : ChatInput.Length);
    ...
    private static int GetCharPosFromBytePos(string text, int bytePos)
    {
    ...
    private static int GetBytePosFromCharPos(string text, int charPos)
    {
        ...
        return Encoding.UTF8.GetByteCount(text.AsSpan(0, charPos));
    }
}