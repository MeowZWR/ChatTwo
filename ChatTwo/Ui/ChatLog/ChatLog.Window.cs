...
public partial class ChatLog : Window, IChatWindow
{
        ...
        else if (activeTab is { Channel: { } channel })
        {
            ...
            else
            {
                ...
                else if (channel.IsMareLinkshell())
                {
                    ...
                    channelNameChunks = BuildMareChannelNameChunks(GetMareChannelLabel(idx));
                }
                else
                {
    ...
    private string GetMareChannelLabel(int idx)
    {
        var mareName = Plugin.MareChat?.GetChannelName(idx) ?? "";
        return string.IsNullOrEmpty(mareName) ? $"\uE044 [{idx + 1}]" : $"\uE044 [{idx + 1}]: {mareName}";
    }

    private Chunk[] BuildMareChannelNameChunks(string text)
    {
        var ime = Plugin.Functions.Chat.GetImePrefixChunks();
        if (ime.Count == 0)
            return [new TextChunk(ChunkSource.None, null, text)];
