            ...
            if (info.Channel is InputChannel.Tell)
            {
                if (info.Rotate != RotateMode.None)
                {
                    ...
                    if (tellInfo != null)
                    {
                        ...
                        if (Sheets.IsInForay())
                        {
                            var worldName = Sheets.WorldSheet.TryGetRow((uint) tellInfo.World, out var world) ? world.Name.ExtractText() : string.Empty;
                ...
                else
                {
                    ...
                    if (target != null)
                    {
                        ...
                        if (target.Reason == 0)
                            resolvedTarget = new TellTarget(target.Name, target.World, target.ContentId, TellReason.Reply);

                        if (info.Permanent)
                        {
                            Plugin.CurrentTab.CurrentChannel.TellTarget = resolvedTarget;
                        }
                        else
                        {
                            Plugin.CurrentTab.CurrentChannel.UseTempChannel = true;
                            Plugin.CurrentTab.CurrentChannel.TempTellTarget = resolvedTarget;
                        }

                        if (Sheets.IsInForay())
                        {
                            var worldName = target.ToWorldString();
                            Plugin.Functions.Chat.SetEurekaTellChannel(target.Name, worldName, (ushort) target.World, 0, target.ContentId, 0, info.Permanent);
                        }
                    }
                }
            }
